...
#include <progrep.h> //Step 1 - Include progrep API (NOTE: For C++ code, header will be cprogrep.h)
...
void main()
{
   ...
   progrep_init(NULL,NULL); //Step 2 - Initialize progrep server. ALTERNATIVE: progrep_init(&total_steps,NULL);

// ONLY IF FIRST PARAMETER IN STEP 2 IS NULL - Assign progrep_tot the total number of iterations before entering main loop
...
// # COMPILATION #
// gcc -o runme progrep_sample.c -lprogrep -lgfortran  # Compile code and link with progrep and gfortran libraries (assuming prog-      
